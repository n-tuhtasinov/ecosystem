...
public class DangerousObjectTypeController {
    ...
    private final DangerousObjectTypeService service;

    @PostMapping
    public ResponseEntity<?> create(@RequestBody DangerousObjectTypeDto dto) {
        try {
            service.create(dto);
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable Integer id, @RequestBody DangerousObjectTypeDto dto) {
        try {
